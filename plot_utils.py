...
def spaced_samples(lower, upper, num_samples):
    return [math.floor(lower + x*(upper-lower)/num_samples) for x in range(num_samples)]
...
def plot_2d_arc(solution, state_labels, chart_label):
    ...
    for idx, ax in enumerate(axs):
        ...
        plt_color_indices = spaced_samples(0, len(color_map.colors), j[-1] + 1) #j is 0 indexed
...
def plot_all_positions(solutions, level, dim_0, dim_1, label_0, label_1, chart_label):
    ...
    for solution in solutions:
        ...
        if good:
           ...
           relevant_input_signal = [value for time, value in input if time <= last_solve_point_time]
...
def plot_solutions_combined(solution_list, level, dim_0, dim_1, label_0, label_1, chart_label):
