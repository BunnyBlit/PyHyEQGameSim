...
class FlappySim():
    ...
    def __init__(self, t_max: float, step_time: float, seed:Optional[int]=None):
        ...
        self.system_params = FlappyParams(pressed_x_vel=2.0, pressed_y_vel=2.0, gamma=9.81)
        self.t_max = t_max
    ...
    def reachability_simulation(self) -> Tuple[List[HybridResult], List[HybridResult]]:
        ...
        start = time.time()
        # upper bound calc
        upper_input_gen = btn_1_ordered_sequence_generator(self.t_max, self.step_time, "dsc")
        upper_solutions = self.find_reachability_bound_given_order(upper_input_gen)
        #upper_solutions = []
        #print("Done!")
        print("... starting on lower bound ...")
        # lower bound calc
        lower_input_gen = btn_1_ordered_sequence_generator(self.t_max, self.step_time, "asc")
    ...
    def find_reachability_bound_given_order(self, input_generator:Generator[InputSignal, Optional[List], None]) -> List[HybridResult]:
        ...
        while not done:
            ...
            if not input_sequence:
                input_sequence = input_generator.send(None) #explicit about getting the first element from the generator
            model.input_sequence = input_sequence
            print(f"Simulating: {''.join([str(sample) for sample in model.input_sequence.samples])}")
            solver = HyEQSolver(model)
            solution = solver.solve()
            ...
            if solver.stop == True:
                ...
                relevant_input = [sample for time, sample in input_sequence if time <= last_sim_time]
                try:
                    # time to skip some!
            ...
            skip_stop_time = time.time()
            if not done:
                print(f"Time spent solving: {solve_stop_time - single_run_start:0.02f}s")
    ...
    def print_reachability_report(self, start:float, stop:float, upper_solutions:List, lower_solutions:List) -> None:
        ...
        print(f"... per run time: {(stop - start) / (len(upper_solutions) + len(lower_solutions)):0.02f}")
